	...
	<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","background":"#ffffff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0px 0px 2rpx 0px","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="select">
				...
				<picker :style='{"width":"100%","flex":"1","height":"auto"}' @change="teacheraccountChange" :value="teacheraccountIndex"  :range="teacheraccountOptions">
					<view :style='{"width":"100%","lineHeight":"80rpx","fontSize":"28rpx","color":"#666666"}' class="uni-input">{{teacheraccountOptions[teacheraccountIndex]}}</view>
				</picker>
			</view>
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0px 0px 2rpx 0px","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			// 下2
			res = await this.$api.option(`teacher`,`teacheraccount`,{});
			this.teacheraccountOptions = res.data;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`mystudent`, this.ruleForm.id);
		...
		methods: {
			...
			// 下二随
			async teacheraccountChange (e) {
				this.teacheraccountIndex = e.target.value
				this.ruleForm.teacheraccount = this.teacheraccountOptions[this.teacheraccountIndex]
				let res = await this.$api.follow(`teacher`, `teacheraccount`,{
					columnValue: this.ruleForm.teacheraccount
				});
				if(res.data.teachername){
					this.ruleForm.teachername = res.data.teachername
				}
			},
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`mystudent`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`mystudent`, this.ruleForm);
						}else{
							await this.$api.add(`mystudent`, this.ruleForm);
						}
