			...
			<view v-if="user&&user.id" :style='{"border":"0px solid #3da742","padding":"24rpx 24rpx 24rpx 24rpx","margin":"0px 0px 20rpx 0px","borderRadius":"0px","background":"none","display":"flex","width":"calc(100% - 0px)","justifyContent":"space-between","height":"auto"}' class="header" v-bind:class="{'status':isH5Plus}" @tap="onPageTap2('../user-info/user-info',1)">
				<view :style='{"padding":"60rpx 100rpx 100rpx 100rpx","alignItems":"flex-start","flexWrap":"wrap","flexDirection":"column","background":"url(http://clfile.zggen.cn/20231204/b6509efad1924ad79ba180867cca2d67.png) no-repeat center top / 100% 100%,#ffffff","display":"flex","width":"calc(96% - 112rpx)","height":"100%"}' v-if="tableName=='student'" class="userinfo">
					...
					<view :style='{"margin":"20rpx 0 0 0","flex":"1","flexDirection":"column","display":"flex","width":"100%","justifyContent":"center","height":"100%"}' class="info">
						<view :style='{"width":"100%","lineHeight":"60rpx","fontSize":"28rpx","color":"#333","fontWeight":"600"}'>{{user.studentaccount}}<text v-if="user.vip&& user.vip=='是'">(VIP)</text></view>
					</view>
				</view>
				<view :style='{"padding":"60rpx 100rpx 100rpx 100rpx","alignItems":"flex-start","flexWrap":"wrap","flexDirection":"column","background":"url(http://clfile.zggen.cn/20231204/b6509efad1924ad79ba180867cca2d67.png) no-repeat center top / 100% 100%,#ffffff","display":"flex","width":"calc(96% - 112rpx)","height":"100%"}' v-if="tableName=='teacher'" class="userinfo">
					<image :style='{"width":"120rpx","padding":"0 0 0 0","margin":"0 auto","borderRadius":"100%","height":"120rpx"}' :src="user.zhaopian?baseUrl+user.zhaopian:'/static/gen/upload.png'"></image>
					<view :style='{"margin":"20rpx 0 0 0","flex":"1","flexDirection":"column","display":"flex","width":"100%","justifyContent":"center","height":"100%"}' class="info">
						<view :style='{"width":"100%","lineHeight":"60rpx","fontSize":"28rpx","color":"#333","fontWeight":"600"}'>{{user.teacheraccount}}<text v-if="user.vip&& user.vip=='是'">(VIP)</text></view>
					</view>
				</view>
		...
		<uni-popup class="popup-content" ref="passwordPopup" type="bottom">
			<view class="passwordForm">
				<view class="passwordInput">
					<input type="password" v-model="passwordForm.password" placeholder="原密码" />
				</view>
				<view class="passwordInput">
					<input type="password" v-model="passwordForm.newpassword" placeholder="新密码" />
				</view>
				<view class="passwordInput">
					<input type="password" v-model="passwordForm.newpassword1" placeholder="确认密码" />
				</view>
		...
		methods: {
			...
			async updatePassword() {
				...
				if (this.tableName=='student') {
					password = this.user.studentpassword;
				}
				let newpassword = this.passwordForm.password
				...
				if (this.tableName=='student') {
					this.user.studentpassword = this.passwordForm.newpassword
				}
