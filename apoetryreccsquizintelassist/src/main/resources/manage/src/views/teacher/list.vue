...
<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('teacher','查看')">
				<div class="overview_header">
					<div>
						...
						<div class="overview_title">教师账号与授课资源统一维护</div>
						<div class="overview_desc">集中查看教师资料、权限状态和可约人数，让教学资源分配更有秩序。</div>
					</div>
				...
				<div class="overview_cards">
					<div class="overview_card">
						...
						<div class="overview_label">教师总数</div>
					</div>
					<div class="overview_card warm">
						<div class="overview_value">{{ teacherStats.enabled }}</div>
						<div class="overview_label">启用账号</div>
					</div>
					<div class="overview_card green">
						<div class="overview_value">{{ teacherStats.reserveTotal }}</div>
						<div class="overview_label">可约总人数</div>
					</div>
					<div class="overview_card blue">
						<div class="overview_value">{{ teacherStats.maleCount }}</div>
						<div class="overview_label">男性教师</div>
					</div>
				</div>
			</div>
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.teacheraccount" placeholder="教师账号"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<div class="btn_view">
					...
					<el-button class="action_btn" v-if=" btnAuth('teacher','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button class="action_btn" type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('teacher','修改')">修改</el-button>
					<el-button class="action_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('teacher','删除')">删除</el-button>
					<el-button class="action_btn stat_btn" type="warning" @click="echartClick1" v-if="btnAuth('teacher','教师人数统计')">教师人数统计</el-button>
				</div>
			</div>
			<el-table
				class="beauty_table"
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('teacher','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button type="success" v-if="btnAuth('teacher','查看')" @click="reserveClick(scope.row)">预约</el-button>
		...
		<el-dialog v-model="reserveVisible" title="预约教师" width="520px" destroy-on-close>
			<el-form ref="reserveFormRef" :model="reserveForm" :rules="reserveRules" label-width="92px">
				<el-form-item label="教师账号">
					<el-input v-model="reserveForm.teacheraccount" readonly />
				</el-form-item>
				<el-form-item label="教师姓名">
					<el-input v-model="reserveForm.teachername" readonly />
				</el-form-item>
				...
				<el-form-item label="人数" prop="reservecount">
					<el-input-number v-model="reserveForm.reservecount" :min="1" :max="999" style="width: 100%;" />
				</el-form-item>
			</el-form>
			<template #footer>
				<span class="formModel_btn_box">
					<el-button class="formModel_cancel" @click="reserveVisible=false">取消</el-button>
					<el-button class="formModel_confirm" type="primary" :loading="reserveSaving" @click="submitReserve">提交预约</el-button>
				</span>
			</template>
		</el-dialog>
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'教师人数统计'" width="70%">
			<div  id="teachernameEchart1" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	const reserveRules = ref({
		...
		reservecount: [{ required: true, message: '请输入预约人数', trigger: 'blur' }]
	})
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.teacheraccount&&searchQuery.value.teacheraccount!=''){
			params['teacheraccount'] = '%' + searchQuery.value.teacheraccount + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
			...
			buildTeacherStats(list.value || [])
		})
	}
	const buildTeacherStats = (rows=[]) => {
		let enabled = 0
		let reserveTotal = 0
		let maleCount = 0
		rows.forEach(item => {
			if ((item.permissionstatus || '启用') !== '禁用') enabled += 1
			reserveTotal += Number(item.reservecount || 0)
			if (item.gender === '男') maleCount += 1
		})
		teacherStats.value = { enabled, reserveTotal, maleCount }
	}
	//删
	const delClick = (id) => {
		let ids = ref([])
	...
	const reserveClick = (row) => {
		if (context?.$toolUtil.storageGet('sessionTable') !== 'student') {
			context?.$toolUtil.message('请使用用户账号登录后预约教师', 'error')
			return
		}
		if ((row.permissionstatus || '启用') === '禁用') {
			context?.$toolUtil.message('当前教师不可预约', 'error')
			return
		}
		reserveForm.value = {
	...
	const submitReserve = () => {
		reserveFormRef.value.validate((valid) => {
			...
			context?.$http({
				url: 'coursereserve/book',
				method: 'post',
				data: reserveForm.value
			}).then(() => {
				context?.$toolUtil.message('预约提交成功，等待教师审核', 'success', () => {
	...
	const togglePermission = (row) => {
		...
		ElMessageBox.confirm(`确定将该教师权限设置为“${nextStatus}”吗？`, '权限控制', {
			...
			context?.$http({
				...
				context?.$toolUtil.message(`教师权限已${nextStatus === '启用' ? '启用' : '禁用'}`, 'success',()=>{
					getList()
				})
			})
		})
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var teachernameEchart1 = echarts.init(document.getElementById("teachernameEchart1"),'macarons');
			context?.$http({
				url: `teacher/group/teachername`,
				method: 'get'
			}).then(res=>{
				...
				// 使用刚指定的配置项和数据显示图表。
				teachernameEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    teachernameEchart1.resize();
				};
			})
		})
	}
