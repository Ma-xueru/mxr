...
<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('feedback','查看')">
				<div class="overview_header">
					<div>
						<div class="overview_badge">意见反馈</div>
						<div class="overview_title">反馈处理进度总览</div>
						<div class="overview_desc">集中查看学生反馈数量、待审核情况和已完成回复记录，方便老师或管理员快速跟进。</div>
					</div>
					<div class="overview_stat">
						<div class="overview_stat_label">当前反馈数</div>
				...
				<div class="overview_cards">
					<div class="overview_card">
						...
						<div class="overview_label">反馈总数</div>
					</div>
					<div class="overview_card warm">
						<div class="overview_value">{{ feedbackStats.pending }}</div>
						<div class="overview_label">待审核</div>
					</div>
					<div class="overview_card green">
						<div class="overview_value">{{ feedbackStats.approved }}</div>
						<div class="overview_label">已通过</div>
					</div>
					<div class="overview_card blue">
						<div class="overview_value">{{ feedbackStats.replied }}</div>
						<div class="overview_label">已回复</div>
					</div>
				</div>
			</div>
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.feedbacktitle" placeholder="反馈标题"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button class="action_btn create_btn" type="success" @click="addClick" v-if="btnAuth('feedback','新增')">新增</el-button>
					<el-button class="action_btn" v-if=" btnAuth('feedback','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button class="action_btn" type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('feedback','修改')">修改</el-button>
					<el-button class="action_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('feedback','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('feedback','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('feedback','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const feedbackStats = ref({ approved: 0, pending: 0, replied: 0 })
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.feedbacktitle&&searchQuery.value.feedbacktitle!=''){
			params['feedbacktitle'] = '%' + searchQuery.value.feedbacktitle + '%'
		}
		...
		context?.$http({
			...
			buildFeedbackStats(list.value || [])
		})
	}
	const buildFeedbackStats = (rows=[]) => {
		let approved = 0
		let pending = 0
		let replied = 0
		rows.forEach(item => {
			...
			if (item.shhf) replied += 1
		})
		feedbackStats.value = { approved, pending, replied }
	}
	//删
	const delClick = (id) => {
		let ids = ref([])
...
.app-contain .beauty_table :deep(.el-tag) { border-radius: 999px !important; padding: 0 12px !important; font-weight: 600 !important; }
</style>
