...
<script setup>
	...
	const ensureAdminMenus = (menuTree) => {
		...
		if (!poetryGroup.child.some(child => child.tableName === 'course')) {
			poetryGroup.child.push({ appFrontIcon: "cuIcon-pay", buttons: ["新增", "查看", "修改", "删除", "查看评论"], menu: "古诗文库", menuJump: "列表", tableName: "course" })
		} else {
			poetryGroup.child = poetryGroup.child.map(child => child.tableName === 'course' ? { ...child, menu: '古诗文库', menuJump: '列表' } : child)
		}
		const classGroup = ensureGroup('班级管理', 'icon-common50', '&#xef96;')
		if (!classGroup.child.some(child => child.tableName === 'classinfo')) {
			classGroup.child.push({ appFrontIcon: "cuIcon-group", buttons: ["新增", "查看", "修改", "删除"], menu: "班级管理", menuJump: "列表", tableName: "classinfo" })
		}
		adminMenu.backMenu = adminMenu.backMenu.map(group => {
			if (group.menu === '管理员管理') group.menu = '用户信息管理'
			if (group.menu === '古诗词管理') {
				group.menu = '古诗文库管理'
				group.child = (group.child || []).map(child => child.tableName === 'course' ? { ...child, menu: '古诗文库', menuJump: '列表' } : child)
			}
			...
			if (group.menu === '用户管理') {
				group.menu = '师生绑定管理'
				group.child = (group.child || []).map(child => child.tableName === 'mystudent' ? { ...child, menu: '师生绑定', menuJump: '列表' } : child)
			}
			return group
		})
		menuTree = menuTree.map(item => ({
			...
			backMenu: (item.backMenu || []).map(group => {
				...
				if (nextGroup.menu === '用户管理') nextGroup.menu = '师生绑定管理'
				if (nextGroup.child) {
					nextGroup.child = nextGroup.child.map(child => child.tableName === 'mystudent' ? { ...child, menu: '师生绑定', menuJump: '列表' } : child)
				}
				return nextGroup
			})
		}))
	...
	const normalizeTeacherTaskMenu = (menuTree) => {
		...
		if (taskGroup && !taskGroup.child.some(child => child.tableName === 'recitationtask')) {
			taskGroup.child.push({ appFrontIcon: "cuIcon-book", buttons: ["新增", "查看", "修改", "删除"], menu: "背诵任务", menuJump: "列表", tableName: "recitationtask" })
		}
		return menuTree
	}
	...
	const login = () => {
		context?.$http({
			url: `sys/login?username=${loginForm.value.username}&password=${loginForm.value.password}`,
			method: 'post'
		}).then(res => {
			context?.$toolUtil.storageSet("Token", res.data.token)
			context?.$toolUtil.storageSet("roleType", res.data.roleType)
			context?.$toolUtil.storageSet("role", res.data.roleType === 'ADMIN' ? '管理员' : '教师')
			context?.$toolUtil.storageSet("sessionTable", res.data.tableName)
