				...
				<el-row>
					<el-col :span="12">
						<el-form-item label="学生账号" prop="studentaccount">
							<el-input class="list_inp" v-model="form.studentaccount" placeholder="学生账号"
					...
					<el-col :span="12">
						<el-form-item label="学生姓名" prop="studentname">
							<el-input class="list_inp" v-model="form.studentname" placeholder="学生姓名"
					...
					<el-col :span="12">
						<el-form-item label="教师账号" prop="teacheraccount">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.teacheraccount?true:false"
								v-model="form.teacheraccount"
								placeholder="请选择教师账号"
								@change="teacheraccountChange">
								<el-option v-for="(item,index) in teacheraccountLists" :label="item" :value="item">
								</el-option>
							</el-select>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		context?.$http({
			url: `option/teacher/teacheraccount`,
			method: 'get'
		}).then(res=>{
			teacheraccountLists.value = res.data.data
		})
		//由上级字段带出不可改
		disabledForm.value.teachername = true;
	}
	...
	const teacheraccountChange=()=>{
		context?.$http({
			url: `follow/teacher/teacheraccount?columnValue=` + form.value.teacheraccount,
			method: 'get'
		}).then(res=>{
			if(res.data.data.teachername){
				form.value.teachername = res.data.data.teachername
			}
		})
	}
	//富文本
