...
<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('mystudent','查看')">
				<div class="overview_header">
					<div>
						<div class="overview_badge">师生分配</div>
						<div class="overview_title">学生与教师绑定关系总览</div>
						<div class="overview_desc">统一查看每位学生对应的负责教师，方便后续布置任务、跟踪辅导与班级协作。</div>
					</div>
					<div class="overview_stat">
						<div class="overview_stat_label">当前分配数</div>
				...
				<div class="overview_cards">
					<div class="overview_card">
						...
						<div class="overview_label">分配记录</div>
					</div>
					<div class="overview_card warm">
						<div class="overview_value">{{ relationStats.studentCount }}</div>
						<div class="overview_label">涉及学生</div>
					</div>
					<div class="overview_card green">
						<div class="overview_value">{{ relationStats.teacherCount }}</div>
						<div class="overview_label">涉及教师</div>
					</div>
					<div class="overview_card blue">
						<div class="overview_value">{{ relationStats.avgLoad }}</div>
						<div class="overview_label">师均学生数</div>
					</div>
				</div>
			</div>
			<div class="list_search_view">
				...
				<div class="btn_view">
					<el-button class="action_btn create_btn" type="success" @click="addClick" v-if="btnAuth('mystudent','新增')">新增</el-button>
					<el-button class="action_btn" v-if=" btnAuth('mystudent','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button class="action_btn" type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('mystudent','修改')">修改</el-button>
					<el-button class="action_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('mystudent','删除')">删除</el-button>
				</div>
			</div>
			<el-table
				class="beauty_table"
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('mystudent','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const relationStats = ref({ studentCount: 0, teacherCount: 0, avgLoad: 0 })
	...
	const getList = () => {
		...
		context?.$http({
			...
			buildRelationStats(list.value || [])
		})
	}
	const buildRelationStats = (rows=[]) => {
		const studentSet = new Set()
		const teacherSet = new Set()
		rows.forEach(item => {
			if (item.studentaccount) studentSet.add(item.studentaccount)
			if (item.teacheraccount || item.teachername) teacherSet.add(item.teacheraccount || item.teachername)
		})
		relationStats.value = {
			studentCount: studentSet.size,
			teacherCount: teacherSet.size,
			avgLoad: teacherSet.size ? Math.round((studentSet.size / teacherSet.size) * 10) / 10 : 0
		}
	}
	//删
	const delClick = (id) => {
