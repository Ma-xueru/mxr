...
<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('admin','查看')">
				<div class="overview_header">
					<div>
						<div class="overview_badge">系统管理员</div>
						<div class="overview_title">后台账号管理总览</div>
						<div class="overview_desc">集中查看管理员账号数量与角色分布，方便统一维护系统权限与后台运营账号。</div>
					</div>
					<div class="overview_stat">
						<div class="overview_stat_label">当前账号数</div>
				...
				<div class="overview_cards">
					<div class="overview_card">
						...
						<div class="overview_label">账号总数</div>
					</div>
					<div class="overview_card warm">
						<div class="overview_value">{{ adminStats.roleCount }}</div>
						<div class="overview_label">角色种类</div>
					</div>
					<div class="overview_card green">
						<div class="overview_value">{{ adminStats.managerCount }}</div>
						<div class="overview_label">管理角色</div>
					</div>
					<div class="overview_card blue">
						<div class="overview_value">{{ adminStats.namedCount }}</div>
						<div class="overview_label">有效用户名</div>
					</div>
				</div>
			</div>
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button class="action_btn create_btn" type="success" @click="addClick" v-if="btnAuth('admin','新增')">新增</el-button>
					<el-button class="action_btn" v-if=" btnAuth('admin','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button class="action_btn" type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('admin','修改')">修改</el-button>
					<el-button class="action_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('admin','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('admin','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const adminStats = ref({ roleCount: 0, managerCount: 0, namedCount: 0 })
	...
	const getList = () => {
		...
		context?.$http({
			...
			buildAdminStats(list.value || [])
		})
	}
	const buildAdminStats = (rows=[]) => {
		const roleSet = new Set()
		let managerCount = 0
		let namedCount = 0
		rows.forEach(item => {
			if (item.role) roleSet.add(item.role)
			if (item.role && item.role.indexOf('管') !== -1) managerCount += 1
			if (item.username) namedCount += 1
		})
		adminStats.value = {
...
.overview_badge {
	...
	padding: 6px 12px;
	border-radius: 999px;
	background: rgba(39, 123, 192, 0.12);
	color: #1c6ca8;
	font-size: 13px;
	font-weight: 700;
	margin-bottom: 10px;
...
.overview_stat {
	...
	box-shadow: inset 0 0 0 1px rgba(210, 225, 236, 0.85);
}
...
.overview_card {
	...
	border-radius: 20px;
	padding: 18px 20px;
	box-shadow: inset 0 0 0 1px rgba(221, 232, 241, 0.95);
}

.overview_card.warm {
	background: linear-gradient(135deg, #fff7eb 0%, #fff1d9 100%);
}

.overview_card.green {
	background: linear-gradient(135deg, #eefbf4 0%, #dcf5e7 100%);
}

.overview_card.blue {
	background: linear-gradient(135deg, #eef6ff 0%, #deedff 100%);
}
...
.list_search_view {
	...
	border: 1px solid rgba(157, 184, 205, 0.3);
	border-radius: 22px;
	padding: 18px 20px !important;
	box-shadow: 0 12px 30px rgba(79, 112, 138, 0.08);
}
...
.action_btn,
.search_btn_view :deep(.el-button) {
	border-radius: 999px !important;
	padding: 10px 22px !important;
	border: none !important;
	box-shadow: 0 10px 22px rgba(72, 114, 146, 0.12);
	font-weight: 600;
}
