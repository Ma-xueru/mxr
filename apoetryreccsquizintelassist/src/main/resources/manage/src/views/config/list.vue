...
<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('config','查看')">
				<div class="overview_header">
					<div>
						<div class="overview_badge">系统配置</div>
						<div class="overview_title">轮播图配置总览</div>
						<div class="overview_desc">展示轮播配置记录、有效图片条目与远程图片数量，便于统一维护首页展示素材。</div>
					</div>
					<div class="overview_stat">
						<div class="overview_stat_label">当前配置数</div>
				...
				<div class="overview_cards">
					<div class="overview_card">
						...
						<div class="overview_label">配置总数</div>
					</div>
					<div class="overview_card warm">
						<div class="overview_value">{{ configStats.withImage }}</div>
						<div class="overview_label">含图片配置</div>
					</div>
					<div class="overview_card blue">
						<div class="overview_value">{{ configStats.remoteImage }}</div>
						<div class="overview_label">远程图片</div>
					</div>
				</div>
			</div>
			<div class="list_search_view">
				<div class="btn_view">
					<el-button class="action_btn" v-if=" btnAuth('config','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button class="action_btn" type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('config','修改')">修改</el-button>
				</div>
			</div>
			<br>
			<el-table
...
<script setup>
	...
	const configStats = ref({ withImage: 0, remoteImage: 0 })
	...
	const getList = () => {
		...
		context?.$http({
			...
			buildConfigStats(list.value || [])
		})
	}
	const buildConfigStats = (rows=[]) => {
		let withImage = 0
		let remoteImage = 0
		rows.forEach(item => {
			if (item.value) withImage += 1
			if (item.value && item.value.substring(0, 4) === 'http') remoteImage += 1
		})
		configStats.value = { withImage, remoteImage }
	}
	//删
	const delClick = (id) => {
		let ids = ref([])
...
.btn_view { display: flex; flex-wrap: wrap; gap: 12px; margin-top: 6px; }
.action_btn { border-radius: 999px !important; padding: 10px 22px !important; border: none !important; box-shadow: 0 10px 22px rgba(72, 114, 146, 0.12); font-weight: 600; }
