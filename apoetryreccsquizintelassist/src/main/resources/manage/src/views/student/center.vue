				...
				<el-row>
					<el-col :span="12">
						<el-form-item label="用户账号" prop="studentaccount">
							<el-input class="list_inp" v-model="user.studentaccount" readonly placeholder="用户账号" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="用户密码" prop="studentpassword">
							<el-input class="list_inp" v-model="user.studentpassword" readonly placeholder="用户密码" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="用户姓名" prop="studentname">
							<el-input class="list_inp" v-model="user.studentname"  placeholder="用户姓名" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="头像" prop="avatar">
							<uploads
								...
								@change="studentavatarUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="性别" prop="gender">
							<el-select 
								...
								placeholder="请选择性别"
								>
								<el-option v-for="item in studentgenderLists" :label="item" :value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="手机号码" prop="telephone">
							<el-input class="list_inp" v-model="user.telephone"  placeholder="手机号码" clearable />
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const tableName = ref('student')
	const user = ref({})
	const studentgenderLists = ref([])
	const init = () => {
		studentgenderLists.value = "男,女".split(',')
	}
	const studentavatarUploadSuccess=(fileUrls)=> {
	    user.value.avatar = fileUrls;
	}
	const onSubmit = () => {
		if((!user.value.studentaccount)){
			context?.$toolUtil.message(`用户账号不能为空`,'error')
			return false
		}
		if((!user.value.studentpassword)){
			context?.$toolUtil.message(`用户密码不能为空`,'error')
			return false
		}
		if((!user.value.studentname)){
		...
		if((user.value.telephone)&&(!context?.$toolUtil.isMobile(user.value.telephone))){
