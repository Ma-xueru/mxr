			...
			<el-form :model="registerForm" class="register_form">
				...
				<div class="list_item">
					...
					<el-input class="list_inp"
						 v-model="registerForm.studentaccount"
						 placeholder="请输入用户账号"
						 type="text"
						/>
				</div>
				...
				<div class="list_item">
					...
					<el-input class="list_inp" v-model="registerForm.studentpassword2" type="password" placeholder="请输入确认用户密码" />
				</div>
				<div class="list_item">
...
<script setup>
	...
	const handleRegister = () => {
		...
		if((!registerForm.value.studentaccount)){
			context?.$toolUtil.message(`用户账号不能为空`,'error')
			return false
		}
		if((!registerForm.value.studentpassword)){
			context?.$toolUtil.message(`用户密码不能为空`,'error')
			return false
		}
		if(registerForm.value.studentpassword!=registerForm.value.studentpassword2){
			context?.$toolUtil.message('两次密码输入不一致','error')
			return false
		}
		if((!registerForm.value.studentname)){
		...
		if(registerForm.value.telephone&&(!context?.$toolUtil.isMobile(registerForm.value.telephone))){
