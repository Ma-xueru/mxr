...
<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('exampaper','查看')">
				<div class="overview_header">
					<div>
						<div class="overview_badge">古诗词测试</div>
						<div class="overview_title">试卷管理总览</div>
						<div class="overview_desc">统一查看试卷数量、发布状态与平均时长，方便老师批量管理测试任务。</div>
					</div>
					<div class="overview_stat">
						<div class="overview_stat_label">当前试卷数</div>
				...
				<div class="overview_cards">
					<div class="overview_card">
						...
						<div class="overview_label">试卷总数</div>
					</div>
					<div class="overview_card warm">
						<div class="overview_value">{{ paperStats.published }}</div>
						<div class="overview_label">已发布</div>
					</div>
					<div class="overview_card green">
						<div class="overview_value">{{ paperStats.unpublished }}</div>
						<div class="overview_label">未发布</div>
					</div>
					<div class="overview_card blue">
						<div class="overview_value">{{ paperStats.avgTime }}</div>
						<div class="overview_label">平均时长(分钟)</div>
					</div>
				</div>
			</div>
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.name" placeholder="古诗词测试名称"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button class="action_btn create_btn" type="success" @click="addClick" v-if="btnAuth('exampaper','新增')">新增</el-button>
					<el-button class="action_btn" v-if=" btnAuth('exampaper','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button class="action_btn" type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('exampaper','修改')">修改</el-button>
					<el-button class="action_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('exampaper','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
...
<script setup>
	...
	const paperStats = ref({ published: 0, unpublished: 0, avgTime: 0 })
	...
	const getList = () => {
		...
		context?.$http({
			...
			buildPaperStats(list.value || [])
		})
	}
	const buildPaperStats = (rows=[]) => {
		let published = 0
		let unpublished = 0
		let timeSum = 0
		rows.forEach(item => {
			if (Number(item.status) === 1 || item.status === '已发布') published += 1
			else unpublished += 1
			timeSum += Number(item.time || 0)
		})
		paperStats.value = {
			published,
			unpublished,
			avgTime: rows.length ? Math.round((timeSum / rows.length) * 10) / 10 : 0
		}
	}
	//删
	const delClick = (id) => {
