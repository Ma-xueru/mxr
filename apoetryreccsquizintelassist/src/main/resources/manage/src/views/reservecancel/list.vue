...
<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('reservecancel','查看')">
				<div class="overview_header">
					<div>
						<div class="overview_badge">取消预约</div>
						<div class="overview_title">取消记录总览</div>
						<div class="overview_desc">查看学生取消预约情况和教师维度分布，便于及时调整课程安排。</div>
					</div>
					<div class="overview_stat">
						<div class="overview_stat_label">当前取消数</div>
				...
				<div class="overview_cards">
					<div class="overview_card">
						...
						<div class="overview_label">取消总数</div>
					</div>
					<div class="overview_card warm">
						<div class="overview_value">{{ cancelStats.studentCount }}</div>
						<div class="overview_label">涉及学生</div>
					</div>
					<div class="overview_card blue">
						<div class="overview_value">{{ cancelStats.teacherCount }}</div>
			...
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button class="action_btn create_btn" type="success" @click="addClick" v-if="btnAuth('reservecancel','新增')">新增</el-button>
					<el-button class="action_btn" v-if=" btnAuth('reservecancel','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button class="action_btn" type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('reservecancel','修改')">修改</el-button>
					<el-button class="action_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('reservecancel','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('reservecancel','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const cancelStats = ref({ studentCount: 0, teacherCount: 0 })
	...
	const getList = () => {
		...
		context?.$http({
			...
			buildCancelStats(list.value || [])
		})
	}
	const buildCancelStats = (rows=[]) => {
		...
		cancelStats.value = { studentCount: studentSet.size, teacherCount: teacherSet.size }
	}
	//删
	const delClick = (id) => {
		let ids = ref([])
...
.beauty_table {
	...
	:deep(.el-button) { border-radius: 999px !important; }
}
</style>
