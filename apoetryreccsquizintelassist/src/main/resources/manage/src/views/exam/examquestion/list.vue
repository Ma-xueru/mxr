<template>
	<div>
		<div class="app-contain">
			<div>
				<div class="overview_panel" v-if="btnAuth('examquestion','查看')">
					<div class="overview_header">
						<div>
							<div class="overview_badge">问答题库管理</div>
							<div class="overview_title">问答题库结构总览</div>
							<div class="overview_desc">快速统计问答题目数量与题型分布，支持老师统一管理题库质量和出题节奏。</div>
						</div>
						<div class="overview_stat">
							<div class="overview_stat_label">当前题目数</div>
					...
					<div class="overview_cards">
						<div class="overview_card">
							...
							<div class="overview_label">题目总数</div>
						</div>
						<div class="overview_card warm">
							<div class="overview_value">{{ questionStats.singleChoice }}</div>
							<div class="overview_label">单选题</div>
						</div>
						<div class="overview_card green">
							<div class="overview_value">{{ questionStats.judge }}</div>
							<div class="overview_label">判断题</div>
						</div>
						<div class="overview_card blue">
							<div class="overview_value">{{ questionStats.fill }}</div>
							<div class="overview_label">填空题</div>
						</div>
					</div>
				</div>
				<div class="list_search_view">
					...
					<br>
					<div class="btn_view">
						<el-button class="action_btn create_btn" type="success" @click="addClick"
							v-if="btnAuth('examquestion','新增')">新增</el-button>
						<el-button class="action_btn" type="danger" :disabled="selRows.length?false:true"
							@click="delClick(null)" v-if="btnAuth('examquestion','删除')">删除</el-button>
						<el-button class="action_btn" type="warning" @click="exportClick" v-if="btnAuth('examquestion','导出')">导出</el-button>
						<el-button class="action_btn" type="primary" @click="printClick" v-if="btnAuth('examquestion','打印')">打印</el-button>
					</div>
				</div>
			</div>
			<br>
...
<script setup>
	...
	const selRows = ref([])
	const questionStats = ref({ singleChoice: 0, judge: 0, fill: 0 })
	//多选
	...
	const getList = () => {
		...
		context?.$http({
			...
			buildQuestionStats(list.value || [])
		})
	}
	const buildQuestionStats = (rows=[]) => {
		let singleChoice = 0
		let judge = 0
		let fill = 0
		rows.forEach(item => {
			if (Number(item.type) === 0) singleChoice += 1
			if (Number(item.type) === 2) judge += 1
			if (Number(item.type) === 3) fill += 1
		})
		questionStats.value = { singleChoice, judge, fill }
	}
