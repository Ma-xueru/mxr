...
<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('news','查看')">
				<div class="overview_header">
					<div>
						<div class="overview_badge">公告信息</div>
						<div class="overview_title">内容发布概览</div>
						<div class="overview_desc">用于快速查看公告数量、图文覆盖情况和标题完整度，方便统一运营管理。</div>
					</div>
					<div class="overview_stat">
						<div class="overview_stat_label">当前公告数</div>
				...
				<div class="overview_cards">
					<div class="overview_card">
						...
						<div class="overview_label">总公告数</div>
					</div>
					<div class="overview_card warm">
						<div class="overview_value">{{ newsStats.withImage }}</div>
						<div class="overview_label">含图公告</div>
					</div>
					<div class="overview_card blue">
						<div class="overview_value">{{ newsStats.titled }}</div>
						<div class="overview_label">完整标题</div>
					</div>
				</div>
			</div>
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button class="action_btn create_btn" type="success" @click="addClick" v-if="btnAuth('news','新增')">新增</el-button>
					<el-button class="action_btn" v-if=" btnAuth('news','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button class="action_btn" type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('news','修改')">修改</el-button>
					<el-button class="action_btn" type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('news','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
...
<script setup>
	...
	const newsStats = ref({ withImage: 0, titled: 0 })
	...
	const getList = () => {
		...
		context?.$http({
			...
			buildNewsStats(list.value || [])
		})
	}
	const buildNewsStats = (rows=[]) => {
		let withImage = 0
		let titled = 0
		rows.forEach(item => {
			if (item.picture) withImage += 1
			if (item.title) titled += 1
		})
		newsStats.value = { withImage, titled }
	}
	//删
	const delClick = (id) => {
		let ids = ref([])
...
.beauty_table {
	...
	:deep(.el-button), :deep(.el-image) { border-radius: 999px; overflow: hidden; }
}
</style>
