<template>
	<div>
		<div class="app-contain">
			<div class="overview_panel" v-if="btnAuth('recitationtask','查看')">
			...
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('recitationtask','新增')">创建作业 / 批量发送</el-button>
					<el-button type="info" v-if="btnAuth('recitationtask','查看')" :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if="btnAuth('recitationtask','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)" v-if="btnAuth('recitationtask','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table v-loading="listLoading" border :stripe='false' @selection-change="handleSelectionChange" ref="table" v-if="btnAuth('recitationtask','查看')" :data="list" @row-click="listChange">
				...
				<el-table-column label="操作" width="320" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if="btnAuth('recitationtask','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button
							type="success"
							v-if="sessionTable === 'teacher' && btnAuth('recitationtask','修改') && scope.row.recitationaudio"
