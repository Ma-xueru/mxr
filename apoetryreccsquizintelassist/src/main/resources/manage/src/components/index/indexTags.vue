<template>
	<div id="tags-view-container" class="tags-view-container" :class="themeClass">
	...
	export default {
		...
		data() {
			return {
				...
				selectedTag: {},
				affixTags: [],
				roleName: localStorage.getItem('role') || '管理员'
			}
		},
		computed: {
			visitedViews() {
			...
			themeClass() {
				return this.roleName === '教师' ? 'teacher_theme' : 'admin_theme'
			}
		},
		watch: {
			$route() {
...
<style lang="scss" scoped>
	.tags-view-container {
		...
		border-style: solid;
		border-radius: 20px;
		box-shadow: 0 14px 30px rgba(33, 47, 87, 0.08);
		height: 44px;
	...
	.teacher_theme {
		background: rgba(249, 242, 226, 0.94);
		border-color: rgba(171, 142, 93, 0.16);
		color: #7a6240;

		.tags-view-item.active {
			color: #4d3823;
			background: linear-gradient(135deg, #edcf9a, #d9edc6);
			box-shadow: 0 8px 16px rgba(184, 146, 92, 0.18);
		}
	}
</style>
