<template>
	<view class="page" v-if="list.length" :style="{'bottom':bottomHeight + 'px'}">
		...
		<view class="list" v-if="listShow">
			<scroll-view :scroll-y="true" style="max-height: 60vh;">
				<view class="music" v-for="(item,index) in list" :key="index" :class="musicIndex==index?'musicActive':''"
					...
					<view class="left" v-if="musicIndex==index">
						<image :src="item.picture?baseUrl + item.picture.split(',')[0]:'/static/player/plays.jpg'" mode="aspectFill"></image>
					</view>
					<view class="center">
		...
		<view class="playView">
			<view class="playLeft">
				<image
					:src="musicIndex!= -1&&list.length&&list[musicIndex].picture? baseUrl + list[musicIndex].picture.split(',')[0]:'/static/player/plays.jpg'"
