	...
	<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","background":"#ffffff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0px 0px 2rpx 0px","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"0px solid #eeeeee","padding":"0px 24rpx 0px 24rpx","margin":"0 0 0 0","color":"#666666","borderRadius":"8rpx","flex":"1","background":"#ffffff00","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' v-model="ruleForm.canceltime" placeholder="取消时间" @tap="toggleTab('canceltime')"></input>
			</view>
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0px 0px 2rpx 0px","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
		...
		<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="canceltimeConfirm" ref="canceltime" themeColor="#333333"></w-picker>
	</view>
</view>
</template>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`reservecancel`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					...
					if(o=='canceltime'){
					this.ruleForm.canceltime = obj[o];
					this.ro.canceltime = true;
					continue;
					}
					if(o=='teacheraccount'){
		...
		methods: {
			...
			// 日长控件选择日期时间
			canceltimeConfirm(val) {
				console.log(val)
				this.ruleForm.canceltime = val.result;
				this.$forceUpdate();
			},
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`reservecancel`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`reservecancel`, this.ruleForm);
						}else{
							await this.$api.add(`reservecancel`, this.ruleForm);
						}
