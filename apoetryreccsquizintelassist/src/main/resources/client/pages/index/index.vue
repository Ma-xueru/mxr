...
<view class="content">
	<view :style='{"width":"100%","flexWrap":"wrap","background":"#ffffff","display":"flex","height":"100%"}'>
		...
		<!-- menu -->
		<view v-if="currentFrontMenu.length" class="menu" :style='{"padding":"0px 0px 0px 40rpx","margin":"40rpx 0px 20rpx 0px","borderColor":"#eee","borderRadius":"8rpx","flexWrap":"wrap","background":"#ffffff","borderWidth":"0px","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
			<block v-for="(menu,index) in currentFrontMenu" :key="index">
		...
		<view v-if="tableName=='student' || tableName=='teacher'" :style='{"width":"calc(100% - 48rpx)","margin":"0 24rpx 10rpx","padding":"0","display":"flex","justifyContent":"space-between","gap":"20rpx"}'>
			<button v-if="tableName=='student'" @tap="onPageTap('recitationtask')" :style='{"flex":"1","border":"0","borderRadius":"20rpx","background":"linear-gradient(135deg,#5fb959 0%,#87d17f 100%)","color":"#fff","fontSize":"28rpx","lineHeight":"84rpx","height":"84rpx"}'>进入背诵任务</button>
			<button v-if="tableName=='teacher'" @tap="onPageTap('recitationtask')" :style='{"flex":"1","border":"0","borderRadius":"20rpx","background":"linear-gradient(135deg,#d84fa9 0%,#f08bc8 100%)","color":"#fff","fontSize":"28rpx","lineHeight":"84rpx","height":"84rpx"}'>进入背诵任务</button>
			<button v-if="tableName=='teacher'" @tap="onPageTap('transcript')" :style='{"flex":"1","border":"0","borderRadius":"20rpx","background":"linear-gradient(135deg,#4a90e2 0%,#78b7ff 100%)","color":"#fff","fontSize":"28rpx","lineHeight":"84rpx","height":"84rpx"}'>进入成绩信息</button>
		</view>

		<view v-if="tableName=='student'" :style='{"width":"calc(100% - 48rpx)","margin":"10rpx 24rpx 0","padding":"28rpx","borderRadius":"24rpx","background":"linear-gradient(135deg,#fff9e8 0%,#eef8ff 100%)"}'>
			<view :style='{"display":"flex","justifyContent":"space-between","alignItems":"center","margin":"0 0 20rpx 0"}'>
				<view>
					<view :style='{"fontSize":"34rpx","fontWeight":"600","color":"#333"}'>老师布置的背诵任务</view>
					<view :style='{"fontSize":"24rpx","color":"#666","margin":"8rpx 0 0 0"}'>登录后就能查看并完成自己的任务</view>
				</view>
				<text :style='{"fontSize":"26rpx","color":"#d84fa9"}' @tap="onPageTap('recitationtask')">查看全部</text>
			</view>
			<view v-if="taskList.length">
				<view v-for="item in taskList" :key="item.id" @tap="goTaskDetail(item.id)" :style='{"background":"#ffffff","borderRadius":"20rpx","padding":"24rpx","margin":"0 0 20rpx 0","boxShadow":"0 10rpx 30rpx rgba(0,0,0,0.04)"}'>
					<view :style='{"display":"flex","justifyContent":"space-between","alignItems":"center"}'>
						<view :style='{"fontSize":"30rpx","fontWeight":"600","color":"#333","width":"70%","overflow":"hidden","textOverflow":"ellipsis","whiteSpace":"nowrap"}'>{{ item.tasktitle || '背诵任务' }}</view>
						<view :style='{"padding":"6rpx 18rpx","borderRadius":"30rpx","fontSize":"22rpx","color":"#fff","background":item.completionstatus==='已完成'?'#5fb959':'#f6a623'}'>{{ item.completionstatus || '待完成' }}</view>
					</view>
					<view :style='{"fontSize":"24rpx","lineHeight":"1.7","color":"#666","margin":"14rpx 0"}'>{{ item.taskcontent || '请按老师要求完成背诵任务。' }}</view>
					<view :style='{"display":"flex","justifyContent":"space-between","fontSize":"22rpx","color":"#999"}'>
						<text>老师：{{ item.teachername || '未分配' }}</text>
						<text>截止：{{ formatDate(item.deadline) }}</text>
					</view>
				</view>
			</view>
			<view v-else :style='{"padding":"30rpx 0","textAlign":"center","fontSize":"24rpx","color":"#999"}'>暂时还没有新的背诵任务</view>
		</view>
...
<script>
	...
	export default {
		...
		async onShow() {
			...
			let menus = menu.list();
			this.menuList = menus;
			let role = uni.getStorageSync("appRole") || '用户'
			this.currentFrontMenu = []
			this.menuList.forEach((item) => {
				if(role==item.roleName) {
					item.frontMenu.forEach((item2) => {
						item2.childs = []
						...
						this.currentFrontMenu.push(item2)
					})
				}
			})
            // let res;
			...
			if (table === 'student') {
				res = await this.$api.page('recitationtask', {
					page: 1,
					limit: 3,
					sort: 'deadline',
					order: 'asc'
		...
		methods: {
			...
			goTaskDetail(id) {
				this.$utils.jump(`../recitationtask/detail?id=${id}`);
			},
