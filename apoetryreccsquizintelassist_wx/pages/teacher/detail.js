...
authobj['isAuthcoursereserveShow']= utils.isAuth('teacher','预约')
}else{
authobj['coursereserveShow']= utils.isAuthFront('teacher','预约')
}
...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.teachername,
...
const {
data
} = await detail("teacher",id)
this.setData({
payAuth:utils.isAuthFront('teacher','支付')
})
...
const resUpdate = await update('teacher', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
data["zhaopian"] = data.zhaopian
data['buynumber']=1
wx.setStorageSync('payObject',data);
wx.setStorageSync('paytable','teacher');
...
async reply() {
const detailList = this.data.detailList
const res = await update("teacher", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
