...
Page({
  ...
  async search() {
    ...
    if (this.data.forumtitle) {
      searchForm['forumtitle'] = '%' + this.data.forumtitle + '%'
    }


    let res = {};
    if (this.data.userid) {
      res = await page(`forum`, searchForm);
    } else {
      res = await list(`forum`, searchForm);
    }
  ...
  addTap() {
    ...
    wx.navigateTo({
      url: `/pages/forum/update-and-add`
    })
  },
  searchListHandler(e) {
    this.setData({
  ...
  editBtn(e) {
    ...
    wx.navigateTo({
      url: `/pages/forum/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
    })
  },
  async detailBtn(e) {
    ...
    wx.navigateTo({
      url: `/pages/forum/detail?isAuth=${this.data.isAuthStatus}`
    })
  ...
  async getData() {
    ...
    let isAuthObj = {}
    if (this.data.isAuthStatus) {
      isAuthObj['addAuth'] = true
      isAuthObj['delAuth'] = utils.isAuth("forum", "删除")
      isAuthObj['editAuth'] = utils.isAuth("forum", "修改")
      ...
      isAuthObj['addAuth'] = true
      isAuthObj['delAuth'] = utils.isAuthFront("forum", "删除")
      isAuthObj['editAuth'] = utils.isAuthFront("forum", "修改")
    }
    //前后台权限判断
    ...
    if (this.data.userid) {
      resList = await page("forum", obj)
    } else {
      resList = await list("forum", obj)
    }
    ...
    if (resList.code == 0) {
      ...
      let goodsListData = resList.data.list?.filter(item =>
        ...
        .map(item => {
          item.picture ? item.picture = item.picture.split(",")[0].replace('upload', 'flie') : ""
          return item;
        });
