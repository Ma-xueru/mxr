...
if(o=='picture'){
refobjempty['picturePath']=baseURL+ obj[o].split(",")[0]
ro.picture = true;
continue;
}else{
}
...
if(o=='forumcontent'){
refobjempty["forumcontent"]=obj[o]
ro.forumcontent = true;
continue;
}else{
}
...
picturePath:baseURL+data?.picture?.split(',')[0],
forumtitle: data.forumtitle,
forumcontent: data.forumcontent,
...
this.data.editStatus? getApp().globalData.editorContent=data.forumcontent:""
...
const obj={
picture:this.data.picture?.replace(regex, ""),
forumtitle: this.data. forumtitle,
forumcontent: getApp().globalData.editorContent,
...
let forumcontent=  getApp().globalData.editorContent
ruleForm['forumcontent']=getApp().globalData.editorContent
if(forumcontent==='' ){
wx.showToast({
icon: "none",
title: `帖子内容不能为空`,
...
let corssRes = await list(`forum`, params)
crossoptnum = wx.getStorageSync('statusColumnName');
...
if (ruleForm.id ) {
await update(`forum`, ruleForm)
}
else {
await add(`forum`, ruleForm)
}
}
