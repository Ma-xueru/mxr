...
let res = {};
if(this.data.userid) {
res = await page(`discussforum`, searchForm);
} else {
res = await list(`discussforum`, searchForm);
}
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("discussforum",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/discussforum/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/discussforum/detail?isAuth=${this.data.isAuthStatus}`
})
...
const userId=       getApp().globalData.userInfo.id
        let isAuthObj={}
if(this.data.isAuthStatus){
isAuthObj['addAuth']   = utils.isAuth("discussforum", "新增")
isAuthObj['delAuth']  = utils.isAuth("discussforum", "删除")
isAuthObj['editAuth']   =utils.isAuth("discussforum", "修改")
...
isAuthObj['addAuth']   = utils.isAuthFront("discussforum", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("discussforum", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("discussforum", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("discussforum",obj)
    }else{
         resList= await list("discussforum",obj)
    }
