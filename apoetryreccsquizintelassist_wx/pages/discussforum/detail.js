...
const {
data
} = await detail("discussforum",id)
this.setData({
payAuth:utils.isAuthFront('discussforum','支付')
})
...
const commentRes = await list("discussdiscussforum", commentData)
this.setData({
...
wx.setStorageSync('tableName',"discussforum" )
wx.navigateTo({
url: `/pages/discussdiscussforum/update-and-add`,
})
...
const resUpdate = await update('discussforum', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
'cartForm.tablename': "discussforum",
})
...
wx.setStorageSync('paytable','discussforum');
...
async reply() {
const detailList = this.data.detailList
const res = await update("discussforum", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
