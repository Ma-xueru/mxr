...
authobj['isAuthmystudentShow']= utils.isAuth('student','分配')
}else{
authobj['mystudentShow']= utils.isAuthFront('student','分配')
}
...
const {
data
} = await detail("student",id)
this.setData({
payAuth:utils.isAuthFront('student','支付')
})
...
this.setData({
detailList,
picture: detailList.avatar.split(','),
})
...
const resUpdate = await update('student', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','student');
...
async reply() {
const detailList = this.data.detailList
const res = await update("student", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','student');
