...
let res = {};
if(this.data.userid) {
res = await page(`student`, searchForm);
} else {
res = await list(`student`, searchForm);
}
...
wx.navigateTo({
url: `/pages/student/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
wx.navigateTo({
url: `/pages/student/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/student/detail?isAuth=${this.data.isAuthStatus}`
})
...
const userId=       getApp().globalData.userInfo.id
        let isAuthObj={}
if(this.data.isAuthStatus){
isAuthObj['addAuth']   = utils.isAuth("student", "新增")
isAuthObj['delAuth']  = utils.isAuth("student", "删除")
isAuthObj['editAuth']   =utils.isAuth("student", "修改")
...
isAuthObj['addAuth']   = utils.isAuthFront("student", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("student", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("student", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("student",obj)
    }else{
         resList= await list("student",obj)
    }
...
.map(item => {
item.avatar? item.avatar = item.avatar.split(",")[0].replace('upload','flie'):""
return item;
});
