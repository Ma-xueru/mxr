...
refobjempty["studentpassword"]=obj[o]
ro.studentpassword = true;
continue;
}else{
}
...
if(o=='avatar'){
refobjempty['avatarPath']=baseURL+ obj[o].split(",")[0]
ro.avatar = true;
continue;
}else{
}
...
if(this.data.genderList[this.data.genderIndex]==undefined ){
wx.showToast({
icon: "none",
title: `性别不能为空`,
})
...
const valuetelephone = this.data.telephone;

if (!mobilePattern.test(valuetelephone)) {
wx.showToast({
icon: "none",
title: `手机号码应输入手机格式`,
})
...
let corssRes = await list(`student`, params)
crossoptnum = wx.getStorageSync('statusColumnName');
...
if (ruleForm.id ) {
await update(`student`, ruleForm)
}
else {
await add(`student`, ruleForm)
}
}
