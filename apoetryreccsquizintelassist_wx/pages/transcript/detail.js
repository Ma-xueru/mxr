...
const {
data
} = await detail("transcript",id)
this.setData({
payAuth:utils.isAuthFront('transcript','支付')
})

this.setData({
goodname:data.studentaccount
})
...
const resUpdate = await update('transcript', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','transcript');
...
async reply() {
const detailList = this.data.detailList
const res = await update("transcript", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
