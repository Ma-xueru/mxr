...
this.getData()
 ...
 let obj = {};
//studentaccount筛选
let resInfostudentaccount = await option('student/studentaccount')
    obj["studentaccountOptions"]= resInfostudentaccount.data
this.setData({
    ...obj
})
...
if(this.data.studentaccount){
    searchForm['studentaccount'] = this.data.studentaccount
}
...
let res = {};
if(this.data.userid) {
res = await page(`transcript`, searchForm);
} else {
res = await list(`transcript`, searchForm);
}
...
wx.navigateTo({
url: `/pages/transcript/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("transcript",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/transcript/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/transcript/detail?isAuth=${this.data.isAuthStatus}`
})
...
const userId=       getApp().globalData.userInfo.id
        let isAuthObj={}
if(this.data.isAuthStatus){
isAuthObj['addAuth']   = utils.isAuth("transcript", "新增")
isAuthObj['delAuth']  = utils.isAuth("transcript", "删除")
isAuthObj['editAuth']   =utils.isAuth("transcript", "修改")
...
isAuthObj['addAuth']   = utils.isAuthFront("transcript", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("transcript", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("transcript", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("transcript",obj)
    }else{
         resList= await list("transcript",obj)
    }
