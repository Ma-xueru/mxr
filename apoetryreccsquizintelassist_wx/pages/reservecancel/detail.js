...
const {
data
} = await detail("reservecancel",id)
this.setData({
payAuth:utils.isAuthFront('reservecancel','支付')
})
...
const resUpdate = await update('reservecancel', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','reservecancel');
...
async reply() {
const detailList = this.data.detailList
const res = await update("reservecancel", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
