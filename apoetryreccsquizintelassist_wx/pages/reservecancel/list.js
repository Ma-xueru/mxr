...
let res = {};
if(this.data.userid) {
res = await page(`reservecancel`, searchForm);
} else {
res = await list(`reservecancel`, searchForm);
}
...
wx.navigateTo({
url: `/pages/reservecancel/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("reservecancel",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/reservecancel/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/reservecancel/detail?isAuth=${this.data.isAuthStatus}`
})
...
const userId=       getApp().globalData.userInfo.id
        let isAuthObj={}
if(this.data.isAuthStatus){
isAuthObj['addAuth']   = utils.isAuth("reservecancel", "新增")
isAuthObj['delAuth']  = utils.isAuth("reservecancel", "删除")
isAuthObj['editAuth']   =utils.isAuth("reservecancel", "修改")
...
isAuthObj['addAuth']   = utils.isAuthFront("reservecancel", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("reservecancel", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("reservecancel", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("reservecancel",obj)
    }else{
         resList= await list("reservecancel",obj)
    }
