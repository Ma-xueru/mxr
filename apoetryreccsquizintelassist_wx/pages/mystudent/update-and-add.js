...
//人脸识别
const teacheraccountres = await option('teacher/teacheraccount')
teacheraccountres.data.unshift('请选择教师账号')
this.setData({
teacheraccountList: teacheraccountres.data
})
...
teacheraccountres.data.map((item, index) => {
if (item == data.teacheraccount) {
this.setData({
teacheraccountIndex: index,
teacheraccount: item
})
this.getfollow("teacher","teacheraccount",item,"teachername")
}else if (this.data.teacheraccountList.length == 1) {
this.setData({
...
let  teacheraccount=this.data.teacheraccountList[selectedIndex]
this.setData({
teacheraccountIndex: selectedIndex,
teacheraccount
});
const {
data
} = await follow('teacher/teacheraccount', teacheraccount)
if(data.teachername){
this.setData({
...
if(this.data.teacheraccountList[this.data.teacheraccountIndex]==undefined ){
wx.showToast({
icon: "none",
title: `教师账号不能为空`,
})
...
let corssRes = await list(`mystudent`, params)
crossoptnum = wx.getStorageSync('statusColumnName');
...
if (ruleForm.id ) {
await update(`mystudent`, ruleForm)
}
else {
await add(`mystudent`, ruleForm)
}
}
