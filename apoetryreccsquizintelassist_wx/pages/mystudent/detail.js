...
const {
data
} = await detail("mystudent",id)
this.setData({
payAuth:utils.isAuthFront('mystudent','支付')
})
...
const resUpdate = await update('mystudent', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','mystudent');
...
async reply() {
const detailList = this.data.detailList
const res = await update("mystudent", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
