...
let res = {};
if(this.data.userid) {
res = await page(`mystudent`, searchForm);
} else {
res = await list(`mystudent`, searchForm);
}
...
wx.navigateTo({
url: `/pages/mystudent/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("mystudent",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/mystudent/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/mystudent/detail?isAuth=${this.data.isAuthStatus}`
})
...
const userId=       getApp().globalData.userInfo.id
        let isAuthObj={}
if(this.data.isAuthStatus){
isAuthObj['addAuth']   = utils.isAuth("mystudent", "新增")
isAuthObj['delAuth']  = utils.isAuth("mystudent", "删除")
isAuthObj['editAuth']   =utils.isAuth("mystudent", "修改")
...
isAuthObj['addAuth']   = utils.isAuthFront("mystudent", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("mystudent", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("mystudent", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("mystudent",obj)
    }else{
         resList= await list("mystudent",obj)
    }
