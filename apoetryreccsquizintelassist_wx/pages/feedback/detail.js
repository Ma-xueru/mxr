...
const {
data
} = await detail("feedback",id)
this.setData({
payAuth:utils.isAuthFront('feedback','支付')
})
...
const resUpdate = await update('feedback', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','feedback');
...
async reply() {
const detailList = this.data.detailList
const res = await update("feedback", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
