...
if(this.data.feedbacktitle){
    searchForm['feedbacktitle'] = '%' + this.data.feedbacktitle + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`feedback`, searchForm);
} else {
res = await list(`feedback`, searchForm);
}
...
wx.navigateTo({
url: `/pages/feedback/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("feedback",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/feedback/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/feedback/detail?isAuth=${this.data.isAuthStatus}`
})
...
const userId=       getApp().globalData.userInfo.id
        let isAuthObj={}
if(this.data.isAuthStatus){
isAuthObj['addAuth']   = utils.isAuth("feedback", "新增")
isAuthObj['delAuth']  = utils.isAuth("feedback", "删除")
isAuthObj['editAuth']   =utils.isAuth("feedback", "修改")
...
isAuthObj['addAuth']   = utils.isAuthFront("feedback", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("feedback", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("feedback", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("feedback",obj)
    }else{
         resList= await list("feedback",obj)
    }
