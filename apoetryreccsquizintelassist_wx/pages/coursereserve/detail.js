...
authobj['isAuthreservecancelShow']= utils.isAuth('coursereserve','取消预约')
}else{
authobj['reservecancelShow']= utils.isAuthFront('coursereserve','取消预约')
}
...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.studentname,
...
const {
data
} = await detail("coursereserve",id)
this.setData({
payAuth:utils.isAuthFront('coursereserve','支付')
})
...
predetailList.shangpinpicture = this.data.picture[0]
if (name == "thumbsupnum") {
// 点赞
...
const resUpdate = await update('coursereserve', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','coursereserve');
...
async reply() {
const detailList = this.data.detailList
const res = await update("coursereserve", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','coursereserve');
