...
Page({
  ...
  async search() {
    ...
    let res = {};
    if (this.data.userid) {
      res = await page(`coursereserve`, searchForm);
    } else {
      res = await list(`coursereserve`, searchForm);
    }
  ...
  addTap() {
    ...
    wx.navigateTo({
      url: `/pages/coursereserve/update-and-add`
    })
  },
  searchListHandler(e) {
    this.setData({
    ...
    wx.showModal({
      ...
      complete: async (res) => {
        ...
        if (res.confirm) {
          ...
          const res = await deleteData("coursereserve", [id])
          console.log(res);
          if (res.code == 0) {
            this.getData()
          }
  ...
  editBtn(e) {
    ...
    wx.navigateTo({
      url: `/pages/coursereserve/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
    })
  },
  async detailBtn(e) {
    ...
    wx.navigateTo({
      url: `/pages/coursereserve/detail?isAuth=${this.data.isAuthStatus}`
    })
  ...
  async getData() {
    ...
    let isAuthObj = {}
    if (this.data.isAuthStatus) {
      isAuthObj['addAuth'] = utils.isAuth("coursereserve", "新增")
      isAuthObj['delAuth'] = utils.isAuth("coursereserve", "删除")
      isAuthObj['editAuth'] = utils.isAuth("coursereserve", "修改")
      ...
      isAuthObj['addAuth'] = utils.isAuthFront("coursereserve", "新增")
      isAuthObj['delAuth'] = utils.isAuthFront("coursereserve", "删除")
    ...
    if (this.data.userid) {
      resList = await page("coursereserve", obj)
    } else {
      resList = await list("coursereserve", obj)
    }
    ...
    if (resList.code == 0) {
      ...
      let goodsListData = resList.data.list
